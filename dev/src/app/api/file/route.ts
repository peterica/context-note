...
import { projectRoot, realSafePath } from '@/lib/notePath';
...
export async function GET(request: NextRequest) {
  ...
  try {
    const absolute = await realSafePath(params.project, params.path);
...
export async function PUT(request: NextRequest) {
  ...
  try {
    const absolute = await realSafePath(project, filePath);
...
export async function POST(request: NextRequest) {
  ...
  try {
    const absolute = await realSafePath(project, name);
